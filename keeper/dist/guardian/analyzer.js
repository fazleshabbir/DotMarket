...
function buildPrompt(snapshot, alerts) {
...
async function callGemini(prompt) {
    if (!GEMINI_API_KEY) {
        (0, logger_1.logEvent)('AI_SKIP', 'No GEMINI_API_KEY configured — using deterministic analysis', 'warning');
        return null;
    }
    try {
        ...
        if (!response.ok) {
            ...
            (0, logger_1.logEvent)('AI_ERROR', `Gemini API error: ${response.status} — ${errText.substring(0, 200)}`, 'warning');
            return null;
        }
        const data = await response.json();
        const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
        if (!text) {
            (0, logger_1.logEvent)('AI_ERROR', 'Gemini returned empty response', 'warning');
            return null;
        }
        ...
        (0, logger_1.logEvent)('AI_DIAGNOSIS', `AI analysis: ${analysis.overallHealth} — ${analysis.rootCause}`, analysis.overallHealth);
        return analysis;
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        (0, logger_1.logEvent)('AI_ERROR', `Gemini call failed: ${msg}`, 'warning');
        return null;
    }
}
/** Deterministic fallback when AI is unavailable */
function deterministicAnalysis(snapshot, alerts) {
    const actions = [];
...
async function analyzeHealth(snapshot, alerts) {
    const now = Date.now();
    // Rate limiting
