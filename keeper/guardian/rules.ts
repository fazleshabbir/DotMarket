...
function createAlert(rule: string, severity: Severity, message: string): Alert {
  return {
...
interface Rule {
  name: string;
  evaluate: (snapshot: HealthSnapshot) => Alert | null;
}
...
const rules: Rule[] = [
  ...
  // ── Oracle Freshness ──
  {
    ...
    evaluate: (s) => {
      if (!s.oracleFresh && s.oracleLastUpdate > 10_000) {
...
export function evaluateRules(snapshot: HealthSnapshot): Alert[] {
  const alerts: Alert[] = [];
...
export function worstSeverity(alerts: Alert[]): Severity {
  const order: Severity[] = ['healthy', 'warning', 'degraded', 'critical'];
